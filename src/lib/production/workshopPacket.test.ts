...
describe('buildWorkshopPacket', () => {
  it('organizes parts, labels, sheets and hardware while preserving project traceability', () => {
    const packet = buildWorkshopPacket({
      ...
      cutPlan: [{ code: 'CHAPA-01', width: 2750, height: 1830, material: 'MDP 18mm', pieces: [
        { id: 'mod-bancada:lateral#1', x: 10, y: 20, width: 600, height: 720 },
        { id: 'mod-bancada:lateral#2', x: 620, y: 20, width: 600, height: 720, rotated: true },
      ] }],
    ...
    expect(packet.labels).toHaveLength(2);
    expect(packet.labels[1]).toMatchObject({ partCode: 'mod-bancada:lateral', sheetCode: 'CHAPA-01', position: { rotated: true } });
