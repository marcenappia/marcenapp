  ...
  return {
    ...
    async handle(task) {
      switch (id) {
        ...
        case 'materials': return parts(task)?.length ? complete(id, task, { normalized: true, stage: 'materials', materials: Array.isArray(task.input.materials) ? task.input.materials : [] }) : needsInput(id, task, ['parts']);
        case 'cut_optimization':
          if (!parts(task)?.length) return needsInput(id, task, ['parts']);
          if (!cutPlan(task)?.length) return needsInput(id, task, ['cutPlan'], { warnings: ['O motor de otimização ainda não está conectado; o sistema não pode declarar um plano ótimo por conta própria.'] });
        ...
        case 'cut_audit': {
          const requested = parts(task); const plan = cutPlan(task); if (!requested?.length || !plan?.length) return needsInput(id, task, ['parts', 'cutPlan']);
        ...
        case 'render': {
          const technicalPackage = buildTechnicalRenderPackage(task.input, task.context?.dependencyResults ?? []);
          return technicalPackage ? complete(id, task, { stage: 'render', sceneReady: true, requiresValidatedTechnicalPackage: true, technicalPackage }) : needsInput(id, task, ['technicalPackage/scene + parts']);
        }
