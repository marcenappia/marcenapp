...
function needsInput(agentId: AgentDefinition['id'], task: AgentTask, fields: string[], extra: Partial<AgentResult> = {}): AgentResult {
  return {
    agentId, taskId: task.id, correlationId: task.correlationId, status: 'needs_input', data: { missing: fields },
    blockers: fields.map((field) => `Dado obrigatório ausente: ${field}`), ...extra,
  };
}
...
function engineeringSpec(task: AgentTask): FurnitureEngineeringSpec | null {
  const value = task.input.engineeringSpec;
  if (!value || typeof value !== 'object') return null;
  return value as FurnitureEngineeringSpec;
}
...
export function createAgent(id: AgentDefinition['id'], name: string, capabilities: string[], dependencies: AgentDefinition['id'][] = []): AgentDefinition {
  return {
    id, name, capabilities, dependencies,
    async handle(task) {
      switch (id) {
        case 'project':
          return task.input.clientId && task.input.workName ? complete(id, task, { ready: true, stage: 'project' }) : needsInput(id, task, ['clientId', 'workName']);
        case 'vision':
          return hasImage(task) ? complete(id, task, { stage: 'vision', analysisReady: true, requiresModel: true }, { confidence: 0, warnings: ['Análise visual real ainda depende de um adaptador de visão configurado; nenhuma medida foi inventada.'], assumptions: ['O agente não cria dimensões a partir de uma imagem sem evidência calibrada.'] }) : needsInput(id, task, ['photoUrl, photoUrls, images ou scene']);
        case 'perspective':
          return task.context?.dependencyResults.some((result) => result.agentId === 'vision' && result.status === 'completed') ? complete(id, task, { stage: 'perspective', geometryReady: true, requiresModel: true }, { confidence: 0, warnings: ['Perspectiva ainda requer o adaptador geométrico/visual; o agente não presume pontos de fuga.'] }) : needsInput(id, task, ['resultado do agente vision']);
        case 'measurement':
          return task.input.measurements || task.input.photoUrl ? complete(id, task, { validated: true, stage: 'measurement', source: task.input.measurements ? 'manual' : 'photo' }) : needsInput(id, task, ['measurements ou photoUrl']);
        case 'measurement_prediction':
          return task.context?.dependencyResults.some((result) => result.agentId === 'measurement' && result.status === 'completed') ? complete(id, task, { stage: 'measurement_prediction', predictionReady: true, requiresModel: true }, { confidence: 0, warnings: ['Predição de medidas exige referência/calibração ou modelo de visão; não há extrapolação silenciosa.'] }) : needsInput(id, task, ['resultado do agente measurement']);
        case 'multiview':
          return hasImage(task) ? complete(id, task, { stage: 'multiview', reconciliationReady: true }, { confidence: 0, warnings: ['Conferência multivista real será executada pelo adaptador de visão; divergências deverão bloquear o avanço.'] }) : needsInput(id, task, ['imagens/perspectivas']);
        case 'furniture_engineering': {
          const explicitParts = parts(task);
          if (explicitParts?.length) return complete(id, task, { stage: 'furniture_engineering', engineeringReady: true, parts: explicitParts });
          const spec = engineeringSpec(task);
          if (!spec) return needsInput(id, task, ['parts ou engineeringSpec']);
          const engineered = engineerBasicCarcass(spec);
          if (engineered.blockers.length) return { ...needsInput(id, task, ['correção da engenharia do móvel']), blockers: engineered.blockers, data: { stage: 'furniture_engineering', engineeringReady: false, ...engineered } };
          return complete(id, task, { stage: 'furniture_engineering', engineeringReady: true, ...engineered });
        }
        case 'materials':
          return parts(task)?.length ? complete(id, task, { normalized: true, stage: 'materials' }) : needsInput(id, task, ['parts']);
        case 'cut_optimization':
          if (!parts(task)?.length) return needsInput(id, task, ['parts']);
          if (!cutPlan(task)?.length) return needsInput(id, task, ['cutPlan'], { warnings: ['O motor de otimização reproduzível deve receber as peças e o plano/entradas de fabricação explícitos.'] });
          return complete(id, task, { stage: 'cut_optimization', candidatePlanReady: true, optimized: false }, { warnings: ['Plano recebido é candidato e ainda não foi declarado ótimo sem motor de otimização reproduzível.'] });
        case 'cut_audit': {
          const requested = parts(task); const plan = cutPlan(task);
        ...
        case 'render':
          return task.input.projectId || task.input.scene ? complete(id, task, { sceneReady: true, stage: 'render', requiresValidatedTechnicalPackage: true }) : needsInput(id, task, ['projectId ou scene']);
        case 'quality':
          return task.input.parts && (task.input.measurements || task.input.photoUrl) ? complete(id, task, { validated: true, blockers: [], stage: 'quality' }) : needsInput(id, task, ['parts', 'measurements ou photoUrl']);
        case 'presentation':
          return task.input.projectId && (task.input.scene || task.input.renderUrl) ? complete(id, task, { ready: true, stage: 'presentation' }) : needsInput(id, task, ['projectId', 'scene ou renderUrl']);
        case 'approval':
          return task.input.presentationId || task.input.approved !== undefined ? complete(id, task, { decisionCaptured: true, stage: 'approval' }) : needsInput(id, task, ['presentationId ou approved']);
        case 'inventory':
          return task.input.materials ? complete(id, task, { checked: true, stage: 'inventory' }) : needsInput(id, task, ['materials']);
        case 'production':
          return parts(task) ? complete(id, task, { cutListReady: true, stage: 'production' }) : needsInput(id, task, ['parts']);
        case 'budget':
          return task.input.materials && (task.input.approved === true || task.input.approvalId) ? complete(id, task, { readyForCortecloud: true, stage: 'budget' }) : needsInput(id, task, ['materials', 'approved ou approvalId']);
        case 'customer':
          return task.input.name ? complete(id, task, { customerReady: true, stage: 'customer' }) : needsInput(id, task, ['name']);
        case 'order':
          return task.input.projectId && task.input.budgetId ? complete(id, task, { orderReady: true, stage: 'order' }) : needsInput(id, task, ['projectId', 'budgetId']);
        case 'documents':
          return task.input.documentType && task.input.budgetId ? complete(id, task, { documentReady: true, stage: 'documents' }) : needsInput(id, task, ['documentType', 'budgetId']);
      }
    },
  };
}
