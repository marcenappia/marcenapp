...
const TYPE_PATTERNS: Array<[PrecisionTargetType, RegExp]> = [
  ['door', /\bporta(?:s)?\b/i], ['drawer', /\bgaveta(?:s)?\b/i], ['handle', /\bpuxador(?:es)?\b/i],
  ['shelf', /\bprateleira(?:s)?\b/i], ['panel', /\bpainel(?:s)?\b/i], ['module', /\bm[oó]dulo(?:s)?\b/i],
  ['cabinet', /\barm[aá]rio(?:s)?\b/i],
];

const ORDINALS: Record<string, number> = {
  primeira: 1, primeiro: 1, segunda: 2, segundo: 2, terceira: 3, terceiro: 3, quarta: 4, quarto: 4,
  quinta: 5, quinto: 5, sexta: 6, sexto: 6, sétima: 7, sétimo: 7, oitava: 8, oitavo: 8,
...
function detectType(input: string): PrecisionTargetType {
  ...
  for (const [type, pattern] of TYPE_PATTERNS) if (pattern.test(normalized)) return type;
  return 'unknown';
}
...
function detectValue(input: string, property: string): unknown {
  ...
  if (dimension) {
    ...
    return dimension[2] === 'm' ? number * 1000 : dimension[2] === 'cm' ? number * 10 : number;
  }
  return undefined;
}
...
export function parsePrecisionEdit(input: string, context: PrecisionParseContext = {}): PrecisionEditContract | undefined {
  ...
  const requiresClarification = !selectedObjectId && (resolvedType === 'unknown' || (ordinal === undefined && side === undefined));
  return {
    operation: detectOperation(input), target, property, value, scope, preserve: 'everything_else',
