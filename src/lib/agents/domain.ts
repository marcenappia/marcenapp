...
function unitToMm(value: number, unit?: string): number { const normalizedUnit = unit?.toLocaleLowerCase('pt-BR'); return normalizedUnit === 'm' ? value * 1000 : normalizedUnit === 'cm' ? value * 10 : value; }
function extractDimensionNear(raw: string, patterns: RegExp[]): number | undefined { for (const pattern of patterns) { const match = raw.match(pattern); if (!match) continue; const value = Number(String(match[1]).replace(',', '.')); if (Number.isFinite(value) && value > 0) return unitToMm(value, match[2]); } return undefined; }
export function parseCreateProjectInput(input: Record<string, unknown>, intent?: string): Record<string, unknown> | undefined {
  ...
  let width = dimensions ? unitToMm(Number(dimensions[1].replace(',', '.')), dimensions[2]) : undefined;
  let height = dimensions ? unitToMm(Number(dimensions[3].replace(',', '.')), dimensions[4]) : undefined;
  let depth = dimensions ? unitToMm(Number(dimensions[5].replace(',', '.')), dimensions[6]) : undefined;

  width ??= extractDimensionNear(raw, [/(?:largura|largo|com\s+)?\s*(\d+(?:[.,]\d+)?)\s*(mm|cm|m)?\s*(?:de\s+)?largura/i, /(?:largura|largo)\s*(?:de|:)?\s*(\d+(?:[.,]\d+)?)\s*(mm|cm|m)?/i]);
  height ??= extractDimensionNear(raw, [/(?:altura|alto)\s*(?:de|:)?\s*(\d+(?:[.,]\d+)?)\s*(mm|cm|m)?/i, /(\d+(?:[.,]\d+)?)\s*(mm|cm|m)?\s*(?:de\s+)?altura/i]);
  depth ??= extractDimensionNear(raw, [/(?:profundidade|profundo)\s*(?:de|:)?\s*(\d+(?:[.,]\d+)?)\s*(mm|cm|m)?/i, /(\d+(?:[.,]\d+)?)\s*(mm|cm|m)?\s*(?:de\s+)?profundidade/i]);
