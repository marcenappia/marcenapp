...
export function buildTechnicalRenderPackage(
  input: Record<string, unknown>,
  dependencyResults: Array<{ agentId: string; data?: Record<string, unknown> }> = [],
): TechnicalRenderPackage | null {
  ...
  return {
    ...
    materials: materials.length
      ? materials
      : (materialResult?.materials as Array<Record<string, unknown>> | undefined) ?? [],
    camera,
    context: { ...(context ?? {}), engineering: engineering ?? null },
    referenceImages,
  };
}
