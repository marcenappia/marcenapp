  ...
  return (
    <div className="flex h-screen bg-background font-sans overflow-hidden">
      ...
      <aside className="hidden md:flex w-64 bg-slate-900 text-slate-300 flex-col border-r border-slate-800 z-20 shrink-0 shadow-2xl">
        <div className="p-4 border-b border-slate-800 h-16"><button type="button" onClick={() => setActiveModule('dashboard')} aria-label="Ir para o início do Marcenapp" className="w-full flex items-center gap-3 font-bold text-white text-left rounded-xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500"><img src={logo} alt="Marcenapp" className="w-9 h-9 rounded-xl" /><div className="leading-tight"><span className="tracking-tight text-sm">MARCENAPP</span><p className="text-[10px] text-slate-400 font-normal tracking-tight">Do projeto à produção</p></div></button></div>
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-slate-50 h-full overflow-hidden">
        <header className="bg-white border-b border-slate-200 px-3 sm:px-4 md:px-8 h-16 flex items-center justify-between sticky top-0 z-10 shadow-sm shrink-0">
          <h2 className="text-lg font-extrabold text-slate-800 flex items-center gap-2 truncate min-w-0"><button type="button" onClick={() => setActiveModule('dashboard')} aria-label="Ir para o início do Marcenapp" className="p-2 bg-indigo-50 rounded-lg text-indigo-600 shrink-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500">{activeModule === 'studio' ? <img src={logo} alt="M" className="w-5 h-5 object-contain" /> : <ActiveIcon size={20} className="shrink-0" />}</button><div className="flex flex-col min-w-0"><span className="truncate leading-none">{activeTitle}</span><span className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter truncate">{activeSubtitle}</span></div></h2>
          <div className="relative shrink-0">
            {user ? <>
              <button onClick={() => setShowUserMenu(!showUserMenu)} aria-label="Abrir menu da marcenaria" aria-expanded={showUserMenu} className="flex items-center gap-2 p-2 md:p-1.5 md:pr-3 rounded-xl md:rounded-full bg-slate-100 hover:bg-slate-200 transition-colors border border-slate-200"><EllipsisVertical className="md:hidden" size={20} /><div className="hidden md:flex w-8 h-8 rounded-full bg-indigo-600 items-center justify-center text-white text-sm font-bold shadow-md">{profile?.name?.charAt(0)?.toUpperCase() || <User size={16} />}</div><div className="hidden sm:block text-left"><p className="text-[10px] font-bold text-slate-700 leading-none">{profile?.name || 'Usuário'}</p><p className="text-[8px] text-slate-500 uppercase tracking-tighter">{profile?.company || 'Marcenaria'}</p></div></button>
