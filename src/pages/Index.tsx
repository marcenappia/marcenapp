...
import IaraAssistantLayer from '@/modules/iara/IaraAssistantLayer';
...
const Index = () => {
  ...
  const renderModule = () => {
    switch (activeModule) {
      ...
      case 'producao': return <ProducaoModule project={budgetProject} parts={parts} navigateTo={setActiveModule} />;
  ...
  return <div className="flex h-screen bg-background font-sans overflow-hidden">
    ...
    <main className="flex-1 flex flex-col min-w-0 bg-slate-50 h-full overflow-hidden">
      <header className="bg-white border-b border-slate-200 px-4 md:px-8 h-16 flex items-center justify-between sticky top-0 z-10 shadow-sm shrink-0">
        <div className="flex items-center gap-3 min-w-0"><img src={logo} alt="MARCENAPP" className="md:hidden w-9 h-9 object-contain shrink-0" /><h2 className="text-lg font-extrabold text-slate-800 flex items-center gap-2 truncate"><div className="p-2 bg-indigo-50 rounded-lg text-indigo-600"><ActiveIcon size={20} className="shrink-0" /></div><div className="flex flex-col"><span className="truncate leading-none">{activeModuleData.label}</span><span className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter">{CATEGORY_LABELS[activeModuleData.category].split(' — ')[0]}</span></div></h2></div>
        <div className="flex items-center gap-2"><IaraAssistantLayer open={showIara} onOpenChange={setShowIara} project={budgetProject} contextLabel={iaraContextLabel} onProjectChange={(p) => setBudgetProject(prev => ({ ...prev, ...p }))} /><div className="relative">{user ? <><button onClick={() => setShowUserMenu(!showUserMenu)} className="flex items-center gap-2 p-1.5 pr-3 rounded-full bg-slate-100 hover:bg-slate-200 transition-colors border border-slate-200"><div className="w-8 h-8 rounded-full bg-indigo-600 flex items-center justify-center text-white text-sm font-bold shadow-sm">{profile?.name?.charAt(0)?.toUpperCase() || <User size={16} />}</div><div className="hidden sm:block text-left"><p className="text-[10px] font-bold text-slate-700 leading-none">{profile?.name || 'Usuário'}</p><p className="text-[8px] text-slate-500 uppercase tracking-tighter">Conta</p></div></button>{showUserMenu && <><div className="fixed inset-0 z-40" onClick={() => setShowUserMenu(false)} /><div className="absolute right-0 top-12 z-50 bg-white border border-slate-200 rounded-2xl shadow-2xl p-3 w-56 animate-in zoom-in-95 duration-200"><div className="px-3 py-2 border-b border-slate-100 mb-2"><p className="font-bold text-slate-800 text-sm truncate">{profile?.name || 'Usuário'}</p><p className="text-xs text-slate-500 truncate">{profile?.company || ''}</p></div><button onClick={() => { setShowUserMenu(false); navigate('/perfil'); }} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-slate-700 hover:bg-indigo-50 rounded-xl transition-colors"><Building2 size={16} className="text-indigo-600" /> Meu perfil / Marcenaria</button><button onClick={() => { if (window.confirm('Deseja reiniciar o tutorial completo?')) { localStorage.removeItem('marcenapp_onboarding_seen'); localStorage.removeItem('marcenapp_onboarding_step'); localStorage.removeItem('marcenapp_onboarding_completed'); if (user) supabase.from('profiles').update({ onboarding_step: 0, onboarding_completed: [] }).eq('user_id', user.id).then(() => window.location.reload()); else window.location.reload(); } }} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-slate-700 hover:bg-slate-50 rounded-xl transition-colors"><RotateCcw size={16} className="text-slate-500" /> Reiniciar Tutorial</button><button onClick={signOut} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-red-500 hover:bg-red-50 rounded-xl transition-colors"><LogOut size={16} /> Sair</button></div></>}</> : <button onClick={() => navigate('/auth')} className="flex items-center gap-2 p-2 px-4 rounded-xl bg-indigo-600 text-white text-sm font-bold hover:bg-indigo-700 transition-colors shadow-sm"><LogIn size={16} /> Entrar</button>}</div></div>
      </header>
      <div className="flex-1 overflow-y-auto overflow-x-hidden p-4 md:p-8 pb-24 md:pb-8 scroll-smooth"><div className="max-w-7xl mx-auto">{diaryContext && <div className="mb-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 flex items-start gap-3"><BookOpen size={18} className="mt-0.5 shrink-0 text-amber-600"/><div><p className="text-xs font-black uppercase tracking-wide text-amber-800">Nota trazida da plancheta</p><p className="mt-1 text-sm text-amber-950 whitespace-pre-wrap">{diaryContext.texto}</p><p className="mt-1 text-[10px] text-amber-700">Use esta observação como contexto da etapa atual. Ela não altera automaticamente medidas ou preços.</p></div></div>}{renderModule()}<footer className="mt-10 border-t border-slate-200 pt-5 pb-4"><div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 text-[10px] text-slate-400"><span className="font-bold tracking-wide text-slate-500">MARCENAPP OS</span><span>Projeto, orçamento, produção e corte em um só lugar.</span></div></footer></div></div>
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 px-1 py-1 z-50 flex justify-around items-center pb-safe shadow-[0_-4px_12px_rgba(0,0,0,0.08)]">{mobileModules.map(m => <button key={m.id} id={`mobile-nav-${m.id}`} aria-label={m.label} aria-current={activeModule === m.id ? 'page' : undefined} onClick={() => setActiveModule(m.id)} onKeyDown={(e) => handleMobileKeyDown(e, m.id)} className={`flex flex-col items-center gap-0.5 p-1.5 rounded-xl transition-all flex-1 min-w-0 focus-visible:outline-none ${activeModule === m.id ? 'text-indigo-600' : 'text-slate-400'}`}><div className={`p-1.5 rounded-xl transition-colors ${activeModule === m.id ? 'bg-indigo-50' : 'bg-transparent'}`}><m.icon size={19} strokeWidth={activeModule === m.id ? 2.5 : 2} aria-hidden="true" /></div><span className="text-[8px] font-bold tracking-tight uppercase truncate max-w-full">{m.mobileLabel}</span></button>)}</nav>
    </main>
  </div>;
};
