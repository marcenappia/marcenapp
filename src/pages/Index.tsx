...
const Index = () => {
  ...
  const iaraContextLabel = activeModule === 'orcamento' ? 'Conferir orçamento' : activeModule === 'corte' ? 'Verificar corte' : activeModule === 'diario' ? 'Organizar informações' : activeModule === 'studio' ? 'Conferir projeto' : activeModule === 'elevator' ? 'Conferir planta' : 'Perguntar à IARA';
  const renderModule = () => {
    switch (activeModule) {
    ...
    <div className="flex h-screen bg-background font-sans overflow-hidden">
      ...
      <aside className="hidden md:flex w-64 bg-slate-900 text-slate-300 flex-col border-r border-slate-800 z-20 shrink-0 shadow-2xl">
        <div className="p-4 flex items-center gap-3 font-bold text-white border-b border-slate-800 h-16">
          <img src={logo} alt="MARCENAPP" className="w-9 h-9 object-contain shrink-0" />
          <div className="leading-tight"><span className="tracking-tight text-sm">MARCENAPP</span><p className="text-[10px] text-slate-400 font-normal tracking-tight">OS — Orquestrador Inteligente</p></div>
        </div>
