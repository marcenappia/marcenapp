...
const Index = () => {
  ...
  useEffect(() => {
    ...
    if (moduleData) document.title = `${moduleData.label} | Marcenapp`;
  }, [activeModule]);
  ...
  const handleKeyDown = (e: React.KeyboardEvent, id: string) => {
    ...
    if (e.key === 'ArrowDown') {
      e.preventDefault(); navButtons[(currentIndex + 1) % navButtons.length].focus();
    } else if (e.key === 'ArrowUp') {
      e.preventDefault(); navButtons[(currentIndex - 1 + navButtons.length) % navButtons.length].focus();
    } else if (e.key === 'Home') {
      e.preventDefault(); navButtons[0].focus();
    } else if (e.key === 'End') {
      e.preventDefault(); navButtons[navButtons.length - 1].focus();
    }
  };
  ...
  const handleMobileKeyDown = (e: React.KeyboardEvent, id: string) => {
    ...
    if (e.key === 'ArrowRight' || e.key === 'ArrowDown') {
      e.preventDefault(); mobileButtons[(currentIndex + 1) % mobileButtons.length].focus();
    } else if (e.key === 'ArrowLeft' || e.key === 'ArrowUp') {
      e.preventDefault(); mobileButtons[(currentIndex - 1 + mobileButtons.length) % mobileButtons.length].focus();
    }
  };
    ...
    <div className="flex h-screen bg-background font-sans overflow-hidden">
      ...
      <aside className="hidden md:flex w-64 bg-slate-900 text-slate-300 flex-col border-r border-slate-800 z-20 shrink-0 shadow-2xl">
        <div className="p-4 flex items-center gap-3 font-bold text-white border-b border-slate-800 h-16">
          ...
          <div className="leading-tight"><span className="tracking-tight text-sm">MARCENA<span className="text-indigo-400">PP</span></span><p className="text-[10px] text-slate-400 font-normal tracking-tight">OS — Orquestrador Inteligente</p></div>
        </div>
        <nav className="flex-1 p-3 space-y-6 overflow-y-auto scrollbar-thin">
          {(Object.keys(groupedModules) as ModuleCategory[]).map(cat => (
            <div key={cat} className="space-y-1">
              <h3 className="px-3 text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 flex items-center justify-between">{CATEGORY_LABELS[cat]}<ChevronRight size={10} className="opacity-50" /></h3>
              {groupedModules[cat]!.map(m => (
                <button key={m.id} id={`nav-${m.id}`} aria-label={m.label} aria-current={activeModule === m.id ? 'page' : undefined} onClick={() => setActiveModule(m.id)} onKeyDown={(e) => handleKeyDown(e, m.id)} className={`w-full flex items-center gap-3 p-2.5 rounded-xl transition-all text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 ${activeModule === m.id ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/20' : 'hover:bg-white/5 hover:text-white text-slate-400'}`}>
                  <m.icon size={18} aria-hidden="true" /><span className="font-semibold text-sm">{m.label}</span>
                </button>
              ))}
            </div>
          ))}
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-slate-50 h-full overflow-hidden">
        <header className="bg-white border-b border-slate-200 px-4 md:px-8 h-16 flex items-center justify-between sticky top-0 z-10 shadow-sm shrink-0">
          <h2 className="text-lg font-extrabold text-slate-800 flex items-center gap-2 truncate"><div className="p-2 bg-indigo-50 rounded-lg text-indigo-600"><ActiveIcon size={20} className="shrink-0" /></div><div className="flex flex-col"><span className="truncate leading-none">{activeModuleData.label}</span><span className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter">{CATEGORY_LABELS[activeModuleData.category].split(' — ')[0]}</span></div></h2>
          <div className="relative">
            {user ? <><button onClick={() => setShowUserMenu(!showUserMenu)} className="flex items-center gap-2 p-1.5 pr-3 rounded-full bg-slate-100 hover:bg-slate-200 transition-colors border border-slate-200"><div className="w-8 h-8 rounded-full bg-indigo-600 flex items-center justify-center text-white text-sm font-bold shadow-md">{profile?.name?.charAt(0)?.toUpperCase() || <User size={16} />}</div><div className="hidden sm:block text-left"><p className="text-[10px] font-bold text-slate-700 leading-none">{profile?.name || 'Usuário'}</p><p className="text-[8px] text-slate-500 uppercase tracking-tighter">Conta Pro</p></div></button>
        ...
        <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 px-1 py-1 z-50 flex justify-around items-center pb-safe shadow-[0_-4px_12px_rgba(0,0,0,0.08)]">
          {mobileModules.map(m => <button key={m.id} id={`mobile-nav-${m.id}`} aria-label={m.label} aria-current={activeModule === m.id ? 'page' : undefined} onClick={() => setActiveModule(m.id)} onKeyDown={(e) => handleMobileKeyDown(e, m.id)} className={`flex flex-col items-center gap-0.5 p-2 rounded-xl transition-all flex-1 focus-visible:outline-none ${activeModule === m.id ? 'text-indigo-600' : 'text-slate-400'}`}><div className={`p-1.5 rounded-xl transition-colors ${activeModule === m.id ? 'bg-indigo-50' : 'bg-transparent'}`}><m.icon size={20} strokeWidth={activeModule === m.id ? 2.5 : 2} aria-hidden="true" /></div><span className="text-[9px] font-bold tracking-tight uppercase">{m.mobileLabel}</span></button>)}
        </nav>
        <Footer />
      </main>
    </div>
