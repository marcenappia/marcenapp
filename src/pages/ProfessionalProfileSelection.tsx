import { ArrowLeft, ArrowRight, BriefcaseBusiness, Building2, Factory, HardHat, House, Palette, Ruler, Users } from 'lucide-react';
...
const PROFILE_ICONS: Record<ProfessionalProfile, typeof BriefcaseBusiness> = {
  marceneiro: HardHat,
...
export default function ProfessionalProfileSelection() {
  ...
  const select = (profession: ProfessionalProfile) => {
    navigate(`/auth?mode=signup&profession=${encodeURIComponent(profession)}`);
  };
      ...
      <div className="mx-auto flex min-h-[calc(100vh-4rem)] w-full max-w-5xl flex-col justify-center">
        ...
        <div className="mb-10 max-w-2xl">
          <div className="mb-5 flex items-center gap-3">
            ...
            <span className="text-sm font-black tracking-wide">MARCENAPP</span>
          </div>
        ...
        <section aria-labelledby="profile-options" className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          ...
          {PROFESSIONAL_PROFILES.map((profile) => {
            ...
            return (
              <button key={profile.value} type="button" onClick={() => select(profile.value)} className="group rounded-2xl border border-white/10 bg-white/[0.045] p-5 text-left transition-all hover:-translate-y-0.5 hover:border-white/20 hover:bg-white/[0.07] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-300">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <ArrowRight size={17} className="mt-1 text-slate-600 transition-transform group-hover:translate-x-1 group-hover:text-slate-300" aria-hidden="true" />
                </div>
                <h3 className="mt-5 text-lg font-black">{profile.label}</h3>
        ...
        <div className="mt-8 flex flex-col gap-3 border-t border-white/10 pt-6 sm:flex-row sm:items-center sm:justify-between">
          ...
          <button type="button" onClick={() => navigate('/auth')} className="inline-flex items-center justify-center gap-2 rounded-xl border border-white/15 px-4 py-2.5 text-sm font-bold text-slate-300 transition-colors hover:bg-white/5 hover:text-white">
