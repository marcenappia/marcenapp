    ...
    <main className="min-h-screen bg-[#f7f5f1] px-4 py-8 text-slate-950 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-6xl">
        <header className="mx-auto max-w-3xl text-center">
          <Link to="/" className="text-sm font-black tracking-tight text-orange-600">MARCENAPP</Link>
          <div className="mt-8 inline-flex items-center gap-2 rounded-full bg-orange-50 px-3 py-1.5 text-xs font-bold text-orange-700"><Sparkles size={14} /> Planos para a sua marcenaria</div>
          <h1 className="mt-4 text-4xl font-black tracking-tight sm:text-5xl">Você vende. O MARCENAPP organiza o resto.</h1>
          <p className="mt-4 text-base leading-7 text-slate-500">Escolha o plano, teste por 7 dias e avance para a assinatura quando estiver pronto.</p>
        </header>

        <section className="mt-10 grid gap-5 lg:grid-cols-3">
          {plans.map((plan) => (
            <article key={plan.name} className={`relative rounded-3xl border bg-white p-6 shadow-sm ${plan.featured ? 'border-orange-400 ring-2 ring-orange-100' : 'border-slate-200'}`}>
              ...
              <p className="mt-2 text-xs font-bold text-emerald-700">7 dias grátis · primeira cobrança após o teste</p>
        ...
        <section className="mx-auto mt-8 max-w-4xl rounded-2xl border border-slate-200 bg-white p-5 text-center shadow-sm">
          <p className="text-sm font-bold text-slate-800">7 dias para testar · sem cobrança durante o teste</p>
          <p className="mt-1 text-xs leading-5 text-slate-500">A assinatura recorrente é criada no Asaas com a primeira cobrança programada para o fim do período de teste.</p>
        </section>
