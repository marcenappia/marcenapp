...
const Auth = () => {
  ...
  const recoveryRequested = searchParams.get('type') === 'recovery' || (typeof window !== 'undefined' && window.location.hash.includes('type=recovery'));
  ...
  useEffect(() => { if (user && !recoveryRequested) goAfterAuth(); }, [user, recoveryRequested, goAfterAuth]);
  useEffect(() => { if (recoveryRequested) { setIsReset(false); setIsLogin(false); } }, [recoveryRequested]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (recoveryRequested) {
        if (!user) throw new Error('O link de recuperação expirou ou ainda não foi validado. Abra novamente o e-mail de recuperação.');
        if (password.length < 6) throw new Error('A nova senha deve ter pelo menos 6 caracteres.');
        const { error } = await supabase.auth.updateUser({ password });
        if (error) setError(error.message); else { setSuccess('Senha alterada com sucesso.'); setPassword(''); setTimeout(goAfterAuth, 700); }
      } else if (isReset) {
  ...
  const title = recoveryRequested ? 'Criar nova senha' : 'MARCENA\u003cspan\u003ePP\u003c/span\u003e';

  return <div className="min-h-screen flex items-center justify-center bg-[hsl(var(--sidebar-bg))] px-4"><div className="w-full max-w-sm space-y-8"><div className="text-center"><img src={logo} alt="MARCENAPP" width="88" height="88" className="mx-auto mb-4 h-22 w-22 object-contain" /><h1 className="text-2xl font-bold tracking-tight text-white">MARCENA<span className="text-[hsl(var(--sidebar-active))]">PP</span></h1><p className="mt-1 text-sm text-[hsl(var(--sidebar-text))]">{recoveryRequested ? 'Defina uma nova senha para sua conta' : 'Marcenaria 4.0'}</p></div>{!isReset && !recoveryRequested && <div className="space-y-3"><button type="button" onClick={() => handleOAuth('google')} disabled={loading || oauthLoading !== null} className="flex w-full items-center justify-center gap-3 rounded-xl bg-white py-3 font-semibold text-slate-800 hover:bg-slate-100 disabled:opacity-50">{oauthLoading === 'google' ? <Loader2 className="animate-spin" size={18} /> : <GoogleIcon />}Continuar com Google</button><button type="button" onClick={() => handleOAuth('apple')} disabled={loading || oauthLoading !== null} className="flex w-full items-center justify-center gap-3 rounded-xl border border-white/20 bg-black py-3 font-semibold text-white hover:bg-neutral-900 disabled:opacity-50">{oauthLoading === 'apple' ? <Loader2 className="animate-spin" size={18} /> : <AppleIcon />}Continuar com Apple</button><div className="flex items-center gap-3 text-[11px] uppercase tracking-widest text-[hsl(var(--sidebar-text))]"><span className="h-px flex-1 bg-white/15" />ou com e-mail<span className="h-px flex-1 bg-white/15" /></div></div>}<form onSubmit={handleSubmit} className="space-y-4">{!isLogin && !isReset && !recoveryRequested && <input type="text" placeholder="Nome completo" value={name} onChange={e => setName(e.target.value)} required className="w-full rounded-xl border border-white/20 bg-white/10 px-4 py-3 text-white placeholder:text-white/40" />}<input type="email" placeholder="E-mail" value={email} onChange={e => setEmail(e.target.value)} required={!recoveryRequested && !isReset} value={email} onChange={e => setEmail(e.target.value)} className="w-full rounded-xl border border-white/20 bg-white/10 px-4 py-3 text-white placeholder:text-white/40" />{!isReset && <input type="password" placeholder={recoveryRequested ? 'Nova senha' : 'Senha'} value={password} onChange={e => setPassword(e.target.value)} required minLength={6} className="w-full rounded-xl border border-white/20 bg-white/10 px-4 py-3 text-white placeholder:text-white/40" />}{!recoveryRequested && isLogin && !isReset && <div className="text-right"><button type="button" onClick={() => setIsReset(true)} className="text-xs text-[hsl(var(--sidebar-text))]">Esqueceu a senha?</button></div>}{error && <div className="space-y-2"><p className="rounded-lg bg-red-950/50 p-3 text-sm text-red-400">{error}</p>{isReset && <a href={SUPPORT_LINK} target="_blank" rel="noopener noreferrer" className="block text-center text-xs text-slate-300 underline">Fale com o suporte</a>}</div>}{success && <p className="rounded-lg bg-emerald-950/50 p-3 text-sm text-emerald-400">{success}</p>}<button type="submit" disabled={loading || (recoveryRequested && !user)} className="flex w-full items-center justify-center gap-2 rounded-xl bg-[hsl(var(--sidebar-active))] py-3 font-bold text-white disabled:opacity-50">{loading && <Loader2 className="animate-spin" size={18} />}{recoveryRequested ? 'Alterar senha' : isReset ? 'Enviar Recuperação' : isLogin ? 'Entrar no MARCENAPP' : 'Criar conta e testar grátis'}</button></form>{!recoveryRequested && <p className="text-center text-sm text-[hsl(var(--sidebar-text))]">{isReset ? <button onClick={() => setIsReset(false)} className="font-semibold text-[hsl(var(--sidebar-active))]">Voltar para o login</button> : <>{isLogin ? 'Não tem conta?' : 'Já tem conta?'}{' '}<button onClick={() => setIsLogin(!isLogin)} className="font-semibold text-[hsl(var(--sidebar-active))]">{isLogin ? 'Cadastre-se' : 'Entrar'}</button></>}</p>}</div></div>;
};
export default Auth;
