...
import { Loader2, BriefcaseBusiness, Ruler, Sparkles, Check } from 'lucide-react';
...
const INITIAL_PROFILES = [
  ...
  { id: 'projetista', label: 'Projetista', description: 'Projeto, detalhamento e documentação', icon: Ruler },
...
const Auth = () => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (!profession) { setError('Escolha como você pretende usar o Marcenapp.'); return; }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[hsl(var(--sidebar-bg))] px-4 py-8">
      <div className="w-full max-w-sm space-y-7">
        ...
        <div className="flex items-center gap-3 text-white/40 text-xs"><div className="h-px flex-1 bg-white/15" /><span>Autenticação oficial Marcenapp</span><div className="h-px flex-1 bg-white/15" /></div>
        {isLogin && !isReset && !isRecoveryPath && <><button type="button" onClick={handleGoogleLogin} disabled={loading || googleLoading} className="w-full py-3 rounded-xl bg-white text-gray-900 font-semibold hover:bg-gray-100 transition-all disabled:opacity-50 flex items-center justify-center gap-3">{googleLoading ? <Loader2 className="animate-spin" size={18} /> : <span className="text-lg font-bold">G</span>}{googleLoading ? 'Conectando ao Google...' : 'Continuar com Google'}</button><div className="flex items-center gap-3 text-white/30 text-xs"><div className="h-px flex-1 bg-white/10" /><span>ou entre com e-mail</span><div className="h-px flex-1 bg-white/10" /></div></>}
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {!isLogin && !isReset && !isRecoveryPath && <div className="space-y-2"><p className="text-xs font-semibold text-white/70">Como você pretende usar o Marcenapp?</p><div className="grid grid-cols-1 gap-2">{INITIAL_PROFILES.map(({ id, label, description, icon: Icon }) => { const active = profession === id; return <button key={id} type="button" onClick={() => setProfession(id)} aria-pressed={active} className={`flex items-center gap-3 rounded-xl border p-3 text-left transition-all ${active ? 'border-[hsl(var(--sidebar-active))] bg-[hsl(var(--sidebar-active)/0.12)]' : 'border-white/15 bg-white/5 hover:bg-white/10'}`}><span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${active ? 'bg-[hsl(var(--sidebar-active))] text-white' : 'bg-white/10 text-white/60'}`}><Icon size={18} /></span><span className="min-w-0 flex-1"><span className="block text-sm font-bold text-white">{label}</span><span className="block text-[11px] leading-4 text-white/45">{description}</span></span>{active && <Check size={17} className="text-[hsl(var(--sidebar-active))]" />}</button>; })}</div><p className="text-[10px] text-white/35">Essa escolha só orienta sua experiência inicial. Você não fica preso a ela.</p></div>}
