...
const plans = { start: { name: 'Start', price: 79 }, pro: { name: 'Pro', price: 179 }, business: { name: 'Business', price: 349 } } as const;
...
export default function Checkout() {
  ...
  useEffect(() => { if (!authLoading && !user) navigate(`/auth?plan=${planKey}`, { replace: true }); if (user) setName(String(user.user_metadata?.name ?? '')); }, [authLoading, user, navigate, planKey]);
  ...
  const handleActivate = async (event: React.FormEvent) => {
    ...
    } catch (err) { setError(err instanceof Error ? err.message : 'Não foi possível conectar ao Asaas agora.'); }
    ...
    <main className="min-h-screen bg-[#0b1015] px-4 py-8 text-white sm:px-6 lg:px-8">
      <div className="mx-auto grid max-w-5xl gap-6 lg:grid-cols-[1fr_.8fr] lg:py-12">
        <section className="rounded-3xl border border-white/10 bg-white/[.04] p-7 shadow-2xl sm:p-9">
          ...
          <p className="mt-8 text-xs font-black uppercase tracking-[.18em] text-orange-300">Contratação opcional</p>
          <h1 className="mt-2 text-3xl font-black">Use primeiro. Decida depois.</h1>
          <p className="mt-3 text-sm leading-6 text-slate-400">Você já pode entrar no aplicativo sem pagar. O teste de 7 dias começa no cadastro. A assinatura abaixo é opcional e pode ser ativada quando você decidir continuar.</p>
          <button type="button" onClick={() => navigate('/app')} className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl border border-emerald-400/30 bg-emerald-500/10 px-5 py-3.5 font-black text-emerald-200 hover:bg-emerald-500/15"><ArrowRight size={18} /> Entrar sem contratar agora</button>
          <div className="my-7 flex items-center gap-3 text-xs uppercase tracking-widest text-slate-600"><span className="h-px flex-1 bg-white/10" />ou contrate agora<span className="h-px flex-1 bg-white/10" /></div>
          <form onSubmit={handleActivate} className="space-y-4">
            ...
            <button disabled={loading || Boolean(success)} className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#f4a640] px-5 py-3.5 font-black text-[#15100a] disabled:opacity-60">{loading ? <Loader2 className="animate-spin" size={18} /> : <ArrowRight size={18} />} {loading ? 'Conectando ao Asaas...' : 'Ativar assinatura com teste de 7 dias'}</button>
          </form>
        </section>
        <aside className="h-fit rounded-3xl border border-orange-300/20 bg-orange-300/[.07] p-7">
          <p className="text-xs font-black uppercase tracking-[.18em] text-orange-300">Resumo</p><h2 className="mt-3 text-2xl font-black">Plano {plan.name}</h2>
          ...
          <div className="my-6 h-px bg-white/10" /><ul className="space-y-3 text-sm text-slate-300"><li className="flex gap-2"><Check size={17} className="text-emerald-400" /> 7 dias grátis sem pagamento obrigatório</li><li className="flex gap-2"><Check size={17} className="text-emerald-400" /> Primeira cobrança apenas se você contratar</li><li className="flex gap-2"><ShieldCheck size={17} className="text-emerald-400" /> Cobrança processada pelo Asaas</li><li className="flex gap-2"><Check size={17} className="text-emerald-400" /> Assinatura mensal opcional</li></ul>
          <p className="mt-7 text-xs leading-5 text-slate-500">Se você ativar a assinatura, o Asaas receberá a primeira cobrança na data programada. Se não ativar, o cadastro continua independente da contratação.</p>
        </aside>
      </div>
    </main>
  );
