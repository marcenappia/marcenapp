...
export default function OperationalIntelligence() {
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: dnaRow }, { data: projectRows }, { data: alertRows }, { data: costRows }, { data: hardwareRows }, { data: stockRows }, { data: reqRows }, { data: stageRows }, { data: saleRow }, { data: receivableRows }] = await Promise.all([
      supabase.from('marcenaria_dna').select('user_id,standard_mdf_thickness_mm,back_thickness_mm,minimum_margin_pct,construction_rules').eq('user_id', user.id).maybeSingle(),
      supabase.from('projects').select('id,nome,name,profit_margin,external_material,doors,drawers,modules').eq('user_id', user.id).order('updated_at', { ascending: false }).limit(20),
      supabase.from('operational_alerts').select('id,alert_type,severity,entity_id,message,evidence,suggested_action').eq('user_id', user.id).eq('status', 'open').order('created_at', { ascending: false }).limit(50),
      ...
      supabase.from('hardware_items').select('id,name,category,unit,unit_price,minimum_stock,supplier_id').eq('user_id', user.id).eq('active', true).order('name'),
      supabase.from('hardware_stock').select('hardware_id,quantity_on_hand').eq('user_id', user.id),
      selectedProject ? supabase.from('project_hardware_requirements').select('hardware_id,quantity_required,rule_key').eq('user_id', user.id).eq('project_id', selectedProject) : Promise.resolve({ data: [] }),
      selectedProject ? supabase.from('project_production_stages').select('id,stage_key,stage_name,status,responsible,due_at,blocked_reason').eq('user_id', user.id).eq('project_id', selectedProject).order('due_at') : Promise.resolve({ data: [] }),
      selectedProject ? supabase.from('project_sales').select('sale_price,status').eq('user_id', user.id).eq('project_id', selectedProject).maybeSingle() : Promise.resolve({ data: null }),
      selectedProject ? supabase.from('project_receivables').select('installment_number,amount,due_at,received_at,status').eq('user_id', user.id).eq('project_id', selectedProject).order('installment_number') : Promise.resolve({ data: [] }),
    ]);
  ...
  const addHardwareRule = async () => {
    ...
    const { error } = await supabase.from('marcenaria_dna').upsert({ user_id: user.id, minimum_margin_pct: dna?.minimum_margin_pct ?? null, standard_mdf_thickness_mm: dna?.standard_mdf_thickness_mm ?? null, construction_rules: { ...(dna?.construction_rules ?? {}), hardware_rules: nextRules } }, { onConflict: 'user_id' });
