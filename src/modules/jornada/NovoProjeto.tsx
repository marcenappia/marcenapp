  ...
  return (
    <div className="max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom-2 duration-300">
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-5 md:p-8 space-y-6">
        ...
        {j.erro && <div role="alert" className="rounded-2xl bg-red-50 border border-red-200 p-4 flex items-start gap-3"><p className="flex-1 text-red-700 font-semibold">{j.erro}</p><button type="button" onClick={j.limparErro} aria-label="Fechar aviso" className="text-red-400 hover:text-red-600"><X size={18} /></button></div>}
        {j.retomando ? <div className="space-y-3" role="status" aria-live="polite"><p className="text-slate-500 font-semibold">Buscando sua obra…</p><div className="h-24 rounded-2xl bg-slate-100 animate-pulse" /></div> : renderEtapa()}
      </div>
