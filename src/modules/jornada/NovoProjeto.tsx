...
export const NovoProjeto = ({ projectId, setBudgetProject, navigateTo }: Props) => {
  ...
  const renderEtapa = () => {
    switch (j.etapa) {
      ...
      case 4:
        return j.analise ? (
          <EtapaAnalise analise={j.analise} respostas={j.respostas} setRespostas={j.setRespostas} onNext={j.gerarApresentacao} loading={j.loading === 'gerando'} />
        ) : null;
      case 5:
      case 6:
  ...
  return (
    <div className="max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom-2 duration-300">
      <div className="flex items-center justify-between mb-4">
        <button
          type="button"
          onClick={j.etapa > 1 && j.etapa < 7 ? j.voltar : () => navigateTo('dashboard')}
          className="min-h-[44px] px-3 rounded-xl flex items-center gap-2 text-slate-600 hover:bg-slate-100 font-bold"
          aria-label={j.etapa > 1 ? 'Voltar um passo' : 'Voltar ao início'}
        >
          <ArrowLeft size={20} /> {j.etapa > 1 && j.etapa < 7 ? 'Voltar' : 'Início'}
        </button>
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-5 md:p-8 space-y-6">
        <ProgressoObra etapa={j.etapa} />
        {j.erro && (
          <div role="alert" className="rounded-2xl bg-red-50 border border-red-200 p-4 flex items-start gap-3">
            <p className="flex-1 text-red-700 font-semibold">{j.erro}</p>
