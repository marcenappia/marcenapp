  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 animate-in fade-in duration-300">
      ...
      <button
        ...
        className="w-full min-h-[88px] rounded-3xl bg-indigo-600 hover:bg-indigo-700 text-white flex items-center gap-4 px-6 shadow-xl shadow-indigo-600/25 active:scale-[0.98] transition-all focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-indigo-200"
      >
      ...
      <section aria-labelledby="minhas-obras">
            ...
            <ol className="mt-4 space-y-3">
              {[
                ...
                <li key={t} className="flex items-center gap-3 text-slate-700">
                  ...
                  <I size={18} className="text-indigo-500" /> <span className="font-semibold">{t}</span>
                </li>
              ))}
            </ol>
          </div>
        ...
        <ul className="space-y-3">
          {obras.map((o) => {
            const pct = percentualObra(o.etapa);
            const etapa = ETAPAS_OBRA.find((e) => e.id === o.etapa)?.label;
            const concluida = o.etapa >= 7;
            return (
              <li key={o.id}>
                <button
                  type="button"
