...
import { Plus, Hammer, ChevronRight, Camera, MessageSquareText, Sparkles, Calculator, CreditCard } from 'lucide-react';
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 animate-in fade-in duration-300">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <button type="button" id="btn-creditos" onClick={() => navigateTo('billing')} className="w-full min-h-[88px] rounded-3xl bg-white hover:bg-slate-50 text-slate-900 border border-slate-200 flex items-center gap-4 px-6 shadow-sm hover:shadow-md active:scale-[0.98] transition-all focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-indigo-200"><span className="w-14 h-14 rounded-2xl bg-indigo-50 text-indigo-600 flex items-center justify-center shrink-0"><CreditCard size={28} /></span><span className="text-left"><span className="block text-xl font-black leading-none">CRÉDITOS E PLANOS</span><span className="block text-slate-500 text-sm mt-1">Ver saldo, comprar créditos e consultar planos</span></span><ChevronRight size={24} className="ml-auto text-slate-400" /></button>
      </div>
