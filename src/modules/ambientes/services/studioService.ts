...
export const studioService = {
  generateVisual: async (prompt: string, images?: ImageData[], stylePrompt?: string, decorPrompt?: string, mode: StudioGenerationMode = 'render'): Promise<string | null> => {
  ...
  elevateFloorPlan: async (floorPlan: ImageData, projectRequest: string, dimensions?: { width?: number; height?: number; depth?: number }): Promise<string | null> => {
    ...
    let topology = '';
    try { topology = await callAIText(analysisPrompt, [floorPlan], true); }
    catch { topology = 'Floor-plan topology analysis unavailable. Use the supplied floor plan image itself as the only geometry authority.'; }
...
OUTPUT: one neutral architectural perspective/axonometric reconstruction of the SAME FLOOR PLAN, with no cabinetry or decorative redesign. The result will be used as the base environment for a later Studio design step.`;
  ...
  refineVisual: async (originalImage: string, instructions: string): Promise<string | null> => {
    const image = await imageSourceToData(originalImage);
    ...
    return await callAIImage(finalPrompt, [image]);
  }
};
