...
function technicalPackagePrompt(packageValue?: TechnicalRenderPackage): string {
  ...
  const compact = JSON.stringify({ version: packageValue.version, projectId: packageValue.projectId, geometry: packageValue.geometry, parts: packageValue.parts, materials: packageValue.materials, camera: packageValue.camera, context: packageValue.context });
  return `\nTECHNICAL SOURCE OF TRUTH: ${compact}\nRespect the supplied geometry, proportions, furniture composition, materials and camera/context. Do not invent or simplify structural elements or dimensions.`;
}
export const studioService = {
  generateVisual: async (prompt: string, images?: ImageData[], stylePrompt?: string, decorPrompt?: string, idempotencyKey?: string, technicalPackage?: TechnicalRenderPackage): Promise<string | null> => {
