  ...
  return (
    <div className="relative space-y-4">
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-xl p-4">
        <label htmlFor="studio-description" className="text-xs font-bold text-slate-400 uppercase mb-2 block">
          Descrição do Projeto
          <span className="ml-2 text-[9px] text-indigo-400 normal-case tracking-normal font-normal">(sincronizado com IARA)</span>
        </label>
        <textarea id="studio-description" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Ex: Cozinha estilo industrial 3.2×2.6m com ilha central..." className="w-full bg-slate-900 border border-slate-600 rounded-lg p-3 text-sm h-24 outline-none focus:border-indigo-500 resize-none text-white placeholder:text-slate-500" />
      </div>
      <Studio
        {...studioProps}
        descriptionSlot={
