...
export function EnvironmentAnalysisPanel({ analysis, analyzing, confirming, onAnalyze, onConfirm, onChange }: {
  ...
  if (!analysis && !analyzing) return <Card className="p-4 border-indigo-200 bg-indigo-50/60"><div className="flex items-start gap-3"><div className="rounded-xl bg-indigo-600 text-white p-2"><ScanSearch size={20}/></div><div className="flex-1"><h3 className="font-bold text-slate-900">Análise do ambiente</h3><p className="text-xs text-slate-600 mt-1">Antes de criar o móvel, a IARA lê paredes, cantos, aberturas e pontos elétricos da foto.</p><Button onClick={onAnalyze} className="mt-3 bg-indigo-600 hover:bg-indigo-700 border-none" icon={ScanSearch}>Analisar ambiente</Button></div></div></Card>;
  ...
  return <Card className="p-4 border-slate-200 bg-white shadow-sm">
    ...
    </div><div className="flex items-center justify-between gap-2 mt-2"><span className={`rounded-full border px-2 py-1 text-[10px] font-medium ${statusClass[item.measurementStatus]}`}>Medida {statusLabel[item.measurementStatus]}</span>{item.angleDeg != null && <span className="text-[10px] text-slate-600">Ângulo: {item.angleDeg}°</span>}{item.confidence !== 'high' && <span className="text-[10px] text-amber-700"><CircleAlert size={12} className="inline mr-1"/>confiança {item.confidence}</span>}</div></div>)}</div>
