...
export const StudioWorker = () => {
  ...
  const processCommand = async (osCommand: OSCommand) => {
    ...
    const hasVisualSource = Boolean(command.images?.length || technicalPackage);
    if (!command.prompt || (command.metadata?.origin === 'iara' && !hasVisualSource)) { fail('Comando inválido: Faltam parâmetros obrigatórios ou pacote técnico/visual.'); return; }
