        ...
        <div className="lg:col-span-4 space-y-4">
          <div className="grid grid-cols-2 gap-2">
            <Card className={`p-2 h-32 flex items-center justify-center relative border-dashed border-2 ${isRefining ? 'border-amber-400 bg-amber-50' : 'border-slate-300'}`}>
              {!sketchImage ? <label className="cursor-pointer flex flex-col items-center justify-center w-full h-full hover:bg-slate-50 transition-colors"><Upload className="text-slate-400" size={20} /><span className="text-[10px] text-slate-500 mt-1 text-center font-medium">Rascunho / Imagem</span><input type="file" className="hidden" accept="image/*" onChange={(e) => e.target.files?.[0] && processFile(e.target.files[0], 'sketch')} /></label> : <img src={sketchImage} className="h-full object-contain" alt="Sketch" />}
            ...
            <Card className="p-2 h-32 flex items-center justify-center relative border-dashed border-2 border-slate-300">
              {!envImage ? <label className="cursor-pointer flex flex-col items-center justify-center w-full h-full hover:bg-slate-50 transition-colors"><MapPin className="text-slate-400" size={20} /><span className="text-[10px] text-slate-500 mt-1 text-center font-medium">Ambiente Real</span><input type="file" className="hidden" accept="image/*" onChange={(e) => e.target.files?.[0] && processFile(e.target.files[0], 'env')} /></label> : <img src={envImage} className="h-full object-contain" alt="Environment" />}
              {envImage && <button onClick={() => { resetEnvironmentAnalysis(); setEnvImage(null); }} className="absolute top-1 right-1 bg-white rounded-full p-1 shadow hover:bg-red-50 hover:text-red-500 transition-colors"><X size={10} /></button>}
            </Card>
          </div>
          ...
          {descriptionSlot ? <div className="h-[520px] lg:h-[600px]">{descriptionSlot}</div> : <Card className="p-4 bg-slate-800 border-slate-700 text-white">
            <div className="flex justify-between items-center mb-2"><label className="text-xs font-bold text-slate-400 uppercase">{isRefining ? "Comando de Edição" : "Descrição do Projeto"}</label></div>
            <textarea value={prompt} onChange={e => setPrompt(e.target.value)} placeholder={isRecording ? "Ouvindo..." : (isRefining ? "Ex: Trocar gavetas por prateleiras..." : "Ex: Cozinha estilo industrial...")} className="w-full bg-slate-900 border border-slate-600 rounded-lg p-3 text-sm h-24 outline-none focus:border-indigo-500 resize-none text-white placeholder:text-slate-500" />
