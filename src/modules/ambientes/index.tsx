    ...
    <>
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-in fade-in pb-20 md:pb-0">
        <div className="lg:col-span-4 space-y-4">
          ...
          {envImage && <EnvironmentAnalysisPanel
            analysis={environmentAnalysis}
            analyzing={analyzingEnvironment}
            confirming={confirmingEnvironment}
            onAnalyze={analyzeEnvironmentImage}
          ...
          {descriptionSlot ? (
            ...
            <Card className="p-4 bg-slate-800 border-slate-700 text-white">
              ...
              <div className="flex gap-2 mt-3 overflow-x-auto pb-2 scrollbar-thin">
                {styles.map(s => <button key={s.id} onClick={() => setSelectedStyle(s)} className={`px-2 py-1 rounded text-xs whitespace-nowrap border transition-colors ${selectedStyle.id === s.id ? 'bg-indigo-600 border-indigo-500 text-white' : 'border-slate-600 text-slate-300 hover:bg-slate-700'}`}>{s.label}</button>)}
              </div>
