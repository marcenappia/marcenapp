...
const DiarioModule = ({ project, navigateTo }: Props) => {
  ...
  useEffect(() => {
    const sincronizado = sincronizarLinhaDoTempoProjeto(projectId, project);
    setEntries(sincronizado);
  }, [projectId, project]);
  ...
  return <div className="space-y-5 animate-in fade-in duration-500">
    ...
    <div className="rounded-xl border border-indigo-100 bg-indigo-50/70 px-4 py-3"><div className="flex items-center gap-2 text-sm font-bold text-indigo-900"><Bot size={17}/> Linha do tempo automática</div><p className="mt-1 text-xs text-indigo-800">A plancheta também registra mudanças importantes do projeto: aprovação do orçamento, liberação da produção e geração da lista de peças. Assim você não precisa anotar tudo manualmente.</p></div>
