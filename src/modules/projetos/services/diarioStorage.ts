...
export function registrarEventoSistema(projectId: string, evento: string, texto: string, modulo: DiarioEntrada['modulo'] = 'sistema', createdAt?: string) {
  const atual = carregarDiario(projectId);
  if (atual.some(item => item.origem === 'sistema' && item.evento === evento)) return atual;
  const nova: DiarioEntrada = { id: `evento-${evento}`, projectId, createdAt: createdAt || new Date().toISOString(), tipo: 'nota', texto, importante: true, origem: 'sistema', evento, modulo };
  const proximo = [nova, ...atual].slice(0, 100);
  salvarDiario(projectId, proximo);
  return proximo;
}
...
export function sincronizarLinhaDoTempoProjeto(projectId: string, project: { jornada?: JornadaData } | null | undefined) {
  ...
  if (Array.isArray(jornada.production?.parts) && jornada.production.parts.length > 0) {
    entradas = registrarEventoSistema(projectId, 'lista-pecas-gerada', `Lista de produção atualizada com ${jornada.production.parts.length} item(ns) derivados do orçamento aprovado.`, 'producao', jornada.production.generatedAt || jornada.production.updatedAt);
  }
  return entradas;
}
