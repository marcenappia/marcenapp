...
export default function MinhaMarcenaria({ onClose }: Props) {
  ...
  const addItem = async () => {
    ...
    const payload: Record<string, unknown> = { user_id: user.id, nome: name.trim() };
    if (tab === 'materiais') Object.assign(payload, { preco: price ? Number(price.replace(',', '.')) : null, fornecedor: supplier.trim() || null, origem: 'manual' });
    if (tab === 'fornecedores') Object.assign(payload, { observacoes: supplier.trim() || null });
    if (tab === 'estoque') Object.assign(payload, { nome_item: name.trim(), quantidade: price ? Number(price.replace(',', '.')) : 0 });
  ...
  const uploadDocument = async () => {
    ...
    const { error } = await supabase.from('marcenaria_documentos').insert({ user_id: user.id, nome: file.name, tipo: file.type || 'arquivo', storage_path: path, status: 'recebido', metadata: { origem: 'minha_marcenaria' } });
    setMessage(error ? 'Arquivo enviado, mas não foi possível registrar a referência.' : 'Documento recebido. A IARA poderá usar esta referência depois da leitura.');
    setFile(null); setLoading(false); await load();
  };
