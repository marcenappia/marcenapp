...
import { Activity, Save, ShieldAlert, Sparkles } from 'lucide-react';
...
export default function CreditRules() {
  ...
  const loadRules = async () => {
    ...
    setIsAdmin(true);
    const [{ data: ruleRows, error: ruleError }, { data: skillRows }, { data: runRows }, { data: alertRows }] = await Promise.all([
  ...
  return <section className="space-y-6">
    <div><h2 className="text-xl font-bold text-slate-800">Administração e observabilidade</h2><p className="text-sm text-slate-500">O painel existente agora observa créditos, Skills, execuções da IARA e alertas operacionais sem criar uma administração paralela.</p></div>
