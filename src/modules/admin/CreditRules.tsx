...
export default function CreditRules() {
  ...
  const loadRules = async () => {
    ...
    if (ruleError) setMessage(ruleError.message); else { setRules((ruleRows ?? []) as any[]); setMessage(''); }
    ...
    setSkills((skillRows ?? []) as any[]); setRuns((runRows ?? []) as any[]); setAlerts((alertRows ?? []) as any[]); setLoading(false);
  };
