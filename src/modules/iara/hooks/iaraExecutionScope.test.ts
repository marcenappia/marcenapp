...
describe('IARA execution isolation', () => {
  it('rejects a Project A result after switching to Project B', () => {
    ...
    expect(isIaraCommandForExecution(command('U1', 'A', 'corr-a', 'E1', 'V1', 1), a)).toBe(true);
  });
  ...
  it('rejects the first Project A execution after A → B → A', () => {
    ...
    expect(isIaraCommandForExecution(command('U1', 'A', 'corr-first-a', 'E1', 'V1', 1), secondA)).toBe(false);
    expect(isIaraCommandForExecution(command('U1', 'A', 'corr-second-a', 'E1', 'V1', 3), secondA)).toBe(true);
  });
  ...
  it('rejects a stale result when only the environment changes', () => {
    ...
    expect(isIaraCommandForExecution(command('U1', 'A', 'corr-a', 'Kitchen', 'V1', 1), execution('U1', 'A', 'corr-a', 1, 'Bedroom', 'V1'))).toBe(false);
  });
  ...
  it('rejects a stale result when only the version changes', () => {
    ...
    expect(isIaraCommandForExecution(command('U1', 'A', 'corr-a', 'Kitchen', 'V1', 1), execution('U1', 'A', 'corr-a', 2, 'Kitchen', 'V2'))).toBe(false);
  });
  ...
  it('rejects a stale result after authentication changes', () => {
    ...
    expect(isIaraCommandForExecution(command('U1', 'A', 'corr-a', 'E1', 'V1', 1), firstUser)).toBe(true);
    expect(isIaraCommandForExecution(command('U2', 'A', 'corr-a', 'E1', 'V1', 1), firstUser)).toBe(false);
  });
  ...
  it('accepts the current Project A render identity', () => {
    ...
    expect(isIaraCommandForExecution(command('U1', 'A', 'corr-a', 'E1', 'V1', 7), current)).toBe(true);
  });

  it('rejects a command when generation is absent', () => {
    const current = execution('U1', 'A', 'corr-a', 7);
    expect(isIaraCommandForExecution(command('U1', 'A', 'corr-a', 'E1', 'V1'), current)).toBe(false);
    expect(isIaraCommandExecutionCurrent(command('U1', 'A', 'corr-a', 'E1', 'V1'), { ...current })).toBe(false);
  });

  it('rejects a command when generation differs', () => {
    const current = execution('U1', 'A', 'corr-a', 7);
    expect(isIaraCommandForExecution(command('U1', 'A', 'corr-a', 'E1', 'V1', 6), current)).toBe(false);
    expect(isIaraCommandExecutionCurrent(command('U1', 'A', 'corr-a', 'E1', 'V1', 6), { ...current })).toBe(false);
  });

  it('rejects a command when correlationId differs', () => {
    const current = execution('U1', 'A', 'corr-a', 7);
    expect(isIaraCommandForExecution(command('U1', 'A', 'corr-b', 'E1', 'V1', 7), current)).toBe(false);
    expect(isIaraCommandExecutionCurrent(command('U1', 'A', 'corr-b', 'E1', 'V1', 7), { ...current })).toBe(false);
  });
  ...
  it('keeps text-only render execution identity independent of an image requirement', () => {
    ...
    expect(isIaraCommandForExecution(command('U1', 'A', 'corr-text-render', 'E1', 'V1', 4), textRender)).toBe(true);
  });
  ...
  it('keeps concurrent executions for different projects isolated', () => {
    ...
    expect(isIaraCommandForExecution(command('U1', 'A', 'corr-a', 'E1', 'V1', 1), a)).toBe(true);
    expect(isIaraCommandForExecution(command('U1', 'B', 'corr-b', 'E1', 'V1', 2), b)).toBe(true);
    expect(isIaraCommandForExecution(command('U1', 'A', 'corr-a', 'E1', 'V1', 1), b)).toBe(false);
    expect(isIaraCommandForExecution(command('U1', 'B', 'corr-b', 'E1', 'V1', 2), a)).toBe(false);
  });
  ...
  it('requires the complete user/project/environment/version identity for a command', () => {
    ...
    expect(isIaraCommandForExecution(command('U1', 'A', 'corr-active', 'E1', 'V2', 2), active)).toBe(true);
    expect(isIaraCommandForExecution(command('U1', 'A', 'corr-active', 'E2', 'V2', 2), active)).toBe(false);
    expect(isIaraCommandForExecution(command('U1', 'A', 'corr-active', 'E1', 'V1', 2), active)).toBe(false);
    expect(isIaraCommandForExecution(command('U2', 'A', 'corr-active', 'E1', 'V2', 2), active)).toBe(false);
  });
