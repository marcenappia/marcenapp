...
export function mergeIaraProjectContext(previous: IaraProjectContext | null, next: IaraProjectContextSnapshot): IaraProjectContextSnapshot {
  ...
  const artifacts = [...(previous?.artifacts ?? []), ...next.artifacts].filter((artifact, index, list) => list.findIndex(item => artifactKey(item) === artifactKey(artifact)) === index).slice(-100);
  return { summary: next.summary ?? previous?.summary ?? null, decisions, artifacts, lastCorrelationId: next.lastCorrelationId ?? previous?.lastCorrelationId ?? null };
}

export async function loadIaraContext(scope: IaraContextScope): Promise<IaraContext | null> {
  let query = supabase.from('project_iara_contexts').select('user_id, client_id, project_id, environment_id, version_id, summary, decisions, artifacts, last_correlation_id').eq('user_id', scope.userId);
  ...
  if (!data) return null;
  return { userId: data.user_id, clientId: data.client_id, projectId: data.project_id, environmentId: data.environment_id, versionId: data.version_id, summary: data.summary, decisions: Array.isArray(data.decisions) ? data.decisions as unknown as IaraProjectDecision[] : [], artifacts: Array.isArray(data.artifacts) ? data.artifacts as unknown as IaraProjectArtifact[] : [], lastCorrelationId: data.last_correlation_id };
}

export async function saveIaraContext(scope: IaraContextScope, snapshot: IaraContextSnapshot): Promise<IaraContext> {
  const { data, error } = await supabase.rpc('merge_iara_context', { p_user_id: scope.userId, p_client_id: scope.clientId, p_project_id: scope.projectId, p_environment_id: scope.environmentId, p_version_id: scope.versionId, p_summary: snapshot.summary ?? null, p_decisions: snapshot.decisions as unknown as Json, p_artifacts: snapshot.artifacts as unknown as Json, p_last_correlation_id: snapshot.lastCorrelationId ?? null });
  if (error) throw error;
  if (!data) throw new Error('A IARA não retornou o contexto persistido.');
  const row = data as unknown as { user_id: string; client_id: string | null; project_id: string | null; environment_id: string | null; version_id: string | null; summary: string | null; decisions: Json; artifacts: Json; last_correlation_id: string | null };
  return { userId: row.user_id, clientId: row.client_id, projectId: row.project_id, environmentId: row.environment_id, versionId: row.version_id, summary: row.summary, decisions: Array.isArray(row.decisions) ? row.decisions as unknown as IaraProjectDecision[] : [], artifacts: Array.isArray(row.artifacts) ? row.artifacts as unknown as IaraProjectArtifact[] : [], lastCorrelationId: row.last_correlation_id };
}

export async function loadIaraProjectContext(projectId: string): Promise<IaraProjectContext | null> {
  const userId = (await supabase.auth.getUser()).data.user?.id ?? '';
  const result = await loadIaraContext({ userId, clientId: null, projectId, environmentId: null, versionId: null });
  return result ? { projectId: result.projectId as string, summary: result.summary, decisions: result.decisions, artifacts: result.artifacts, lastCorrelationId: result.lastCorrelationId } : null;
}
