...
import { Maximize2, Loader2, AlertCircle, RefreshCcw, ExternalLink } from 'lucide-react';
  ...
  <main className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin relative">
    {messages.length === 0 && !isTyping && (
      <div className="flex flex-col items-center justify-center text-center py-10 px-4 opacity-80">
        ...
        <p className="text-xs text-muted-foreground max-w-xs leading-relaxed">Descreva seu projeto no campo abaixo — dimensões, estilo, materiais. A descrição fica sincronizada com o Estúdio.</p>
        ...
        <div key={msg.id} className={`flex flex-col ${isUser ? 'items-end' : 'items-start'}`}>
          <div className={`max-w-[85%] sm:max-w-[70%] rounded-2xl p-1 shadow-md ${isUser ? 'bg-primary rounded-tr-sm' : 'bg-card border border-border rounded-tl-sm'}`}>
            {msg.image_url && <div className="relative rounded-xl overflow-hidden mb-1 group cursor-zoom-in" onClick={() => isUser && onImageZoom({ url: msg.image_url!, budget: msg.budget })}>
              ...
              <div className="absolute top-2 right-2"><span className="bg-black/40 text-[8px] text-white px-2 py-0.5 rounded-full uppercase font-bold tracking-widest backdrop-blur-sm">Contexto</span></div>
            </div>}
            ...
            {canOpenArtifact && <div className="px-3 pb-2"><button type="button" onClick={() => onArtifactOpen?.({ type: artifact!.type, id: artifact!.id, imageUrl: msg.image_url })} className={`inline-flex items-center gap-2 px-3 py-2 rounded-lg border text-xs font-bold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary ${isUser ? 'border-primary-foreground/30 text-primary-foreground hover:bg-primary-foreground/10' : 'border-border text-foreground hover:bg-muted'}`}><ExternalLink size={14} /> Abrir {artifact?.type === 'render' ? 'render' : 'artefato'}</button></div>}
    ...
    {error && !isTyping && <div role="alert" className="flex items-start gap-3 bg-destructive/10 border border-destructive/30 rounded-xl p-3 animate-in fade-in"><AlertCircle size={16} className="text-destructive shrink-0 mt-0.5" aria-hidden="true" /><div className="flex-1 min-w-0"><p className="text-xs font-bold text-destructive">Não consegui responder</p><p className="text-[11px] text-muted-foreground break-words">{error}</p><div className="flex gap-2 mt-2">{onRetry && <button onClick={onRetry} className="flex items-center gap-1 px-3 py-1.5 bg-primary text-primary-foreground rounded-lg text-[10px] font-black uppercase tracking-wider hover:opacity-90 transition-opacity"><RefreshCcw size={12} aria-hidden="true" /> Tentar novamente</button>}{onDismissError && <button onClick={onDismissError} className="px-3 py-1.5 text-[10px] font-bold uppercase text-muted-foreground hover:text-foreground transition-colors">Fechar</button>}</div></div></div>}
    {isTyping && <div className="flex items-start"><div role="status" aria-label="IARA processando" className="bg-card border border-border rounded-2xl rounded-tl-sm p-4 shadow-md flex items-center gap-2"><Loader2 size={16} className="animate-spin text-primary" /><span className="text-xs text-muted-foreground">IARA está processando…</span></div></div>}
