  ...
  <footer className="bg-card border-t border-border p-3 shrink-0 relative">
    ...
    <div className="flex items-end gap-2">
      <div className="flex gap-1 bg-muted p-1 rounded-2xl border border-border">
        ...
        <button onClick={toggleRecording} className={`p-2.5 rounded-xl transition-all ${isListening ? 'bg-red-500 text-white animate-pulse' : 'text-muted-foreground hover:text-primary'}`}>
      ...
      <div className="flex-1 relative group">
        ...
        <button onClick={onSend} className="absolute right-2 bottom-1.5 p-2 bg-primary text-primary-foreground rounded-xl shadow-lg shadow-primary/20 hover:scale-105 active:scale-95 transition-all"><Send size={18} /></button>
      </div>
    </div>
  </footer>
);
