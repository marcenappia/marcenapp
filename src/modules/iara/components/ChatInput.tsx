...
export const ChatInput = ({ chatInput, setChatInput, onSend, onImageSelect, toggleRecording, isListening, pendingUpload, setPendingUpload, onSmartAction }: ChatInputProps) => {
  ...
  return (
    <footer className="bg-card border-t border-border p-3 sm:p-4 shrink-0 relative" aria-label="Compositor da IARA">
      ...
      {open && <div role="dialog" aria-label="Ações rápidas da IARA" className="absolute bottom-full left-3 right-3 sm:left-auto sm:right-3 mb-2 w-auto sm:w-[min(420px,calc(100vw-1.5rem))] max-h-[70vh] overflow-y-auto bg-card border border-border rounded-2xl shadow-2xl p-3 z-30"><div className="flex items-center justify-between px-1 pb-2"><div><p className="text-xs font-bold">Ações rápidas</p><p className="text-[10px] text-muted-foreground">Escolha uma ação ou escreva o que precisa.</p></div><button type="button" aria-label="Fechar ações rápidas" onClick={() => setOpen(false)} className="p-2 rounded-lg hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"><X size={16}/></button></div><div className="grid gap-3">{SMART_ACTIONS.map(group => <section key={group.domain}><p className="px-1 mb-1.5 text-[9px] uppercase tracking-widest font-bold text-muted-foreground">{group.title}</p><div className="grid grid-cols-2 gap-1.5">{group.items.map(action => { const Icon = ICONS[action.id] ?? Command; return <button key={action.id} type="button" onClick={() => { setOpen(false); onSmartAction?.(action); }} className="flex items-center gap-2 text-left px-2.5 py-2.5 min-h-11 rounded-xl border border-border hover:border-primary hover:bg-muted transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"><Icon size={15} className="shrink-0 text-muted-foreground" aria-hidden="true"/><span className="text-[11px] font-semibold">{action.label}</span></button>; })}</div></section>)}</div></div>}
      <div className="flex items-end gap-2">
        <div className="flex gap-1 bg-muted p-1 rounded-2xl border border-border shrink-0">
          <label className="p-2.5 min-w-11 min-h-11 rounded-xl hover:bg-background transition-all cursor-pointer text-muted-foreground hover:text-primary inline-flex items-center justify-center focus-within:ring-2 focus-within:ring-primary" aria-label="Anexar imagem"><Paperclip size={18} aria-hidden="true" /><input type="file" className="hidden" accept="image/*" onChange={onImageSelect} /></label>
          ...
          <button type="button" aria-label="Abrir ações rápidas" aria-expanded={open} onClick={() => setOpen(v => !v)} className={`p-2.5 min-w-11 min-h-11 rounded-xl transition-all ${open ? 'bg-background text-primary' : 'text-muted-foreground hover:text-primary'}`}><Command size={18} aria-hidden="true" /></button>
        </div>
        <div className="flex-1 relative group"><textarea aria-label="Mensagem para a IARA" value={chatInput} onChange={(e) => setChatInput(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); onSend(); } }} placeholder={isListening ? 'IARA está ouvindo...' : 'Descreva o que você quer fazer...'} className="w-full bg-muted border border-border rounded-2xl py-3 px-4 pr-12 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all resize-none max-h-32 scrollbar-none min-h-11" rows={1} /><button type="button" aria-label="Enviar mensagem" onClick={onSend} className="absolute right-2 bottom-1.5 min-w-9 min-h-9 p-2 bg-primary text-primary-foreground rounded-xl shadow-sm hover:opacity-90 active:scale-95 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"><Send size={18} aria-hidden="true" /></button></div>
      </div>
    </footer>
  );
};
