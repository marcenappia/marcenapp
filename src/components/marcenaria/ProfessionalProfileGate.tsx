...
/**
 * First-access gate shown after authentication when the account has no
 * professional profile yet. Existing users with a profile go straight to
 * their recurring work area.
 */
const ProfessionalProfileGate = () => {
  ...
  const continueToWorkspace = async () => {
    ...
    try {
      ...
      const firstModule = selected === 'projetista' ? 'novo' : selected === 'outro' ? 'studio' : 'dashboard';
      navigate(`/?module=${firstModule}`, { replace: true });
    } catch (caught) {
      ...
      <div className="w-full max-w-2xl">
        <div className="mb-8 text-center">
          ...
          <p className="mx-auto mt-3 max-w-xl text-sm leading-6 text-slate-300 md:text-base">
            Escolha seu perfil profissional para abrir o Marcenapp já na área de trabalho mais adequada ao seu dia a dia.
          </p>
        </div>
          ...
          {PROFESSIONAL_PROFILES.map(({ id, label, description, icon: Icon }) => {
            ...
            return (
              <button
                ...
                aria-pressed={active}
                className={`group rounded-2xl border p-5 text-left transition-all ${active ? 'border-indigo-400 bg-indigo-500/15 ring-2 ring-indigo-400/30' : 'border-white/10 bg-white/[0.04] hover:border-white/20 hover:bg-white/[0.07]'}`}
              >
                <div className={`flex h-12 w-12 items-center justify-center rounded-xl ${active ? 'bg-indigo-500 text-white' : 'bg-white/10 text-slate-300'}`}>
                  <Icon size={22} />
                </div>
                <div className="mt-5 flex items-start justify-between gap-2">
                  ...
                  {active && <Check size={18} className="shrink-0 text-indigo-300" />}
                </div>
              </button>
            );
          })}
        ...
        <button
          ...
          className="mx-auto mt-6 flex min-h-12 w-full max-w-sm items-center justify-center gap-2 rounded-xl bg-white px-5 py-3 text-sm font-black text-slate-950 transition hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-40"
        >
          {saving && <Loader2 size={17} className="animate-spin" />}
          {saving ? 'Preparando sua área...' : 'Entrar na minha área de trabalho'}
        </button>
