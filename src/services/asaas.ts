...
export function listAsaasCustomers(filters: { limit?: number; offset?: number; externalReference?: string; cpfCnpj?: string; email?: string } = {}) {
  return callAsaas<{ data: AsaasCustomer[]; totalCount: number }>({ action: 'list_customers', ...filters });
}

export function createAsaasCustomer(input: { name: string; cpfCnpj?: string; email?: string; mobilePhone?: string; externalReference?: string }) {
  return callAsaas<{ customer: AsaasCustomer; reused: boolean }>({ action: 'create_customer', ...input });
}

export function createAsaasPayment(input: { customerId: string; value: number; billingType: AsaasBillingType; dueDate?: string; description?: string; externalReference?: string; installmentCount?: number; totalValue?: number }) {
  return callAsaas<{ payment: AsaasPayment }>({ action: 'create_payment', ...input });
}
...
export function createAsaasSubscription(input: { customerId: string; value: number; billingType: AsaasBillingType; cycle: AsaasCycle; nextDueDate?: string; description?: string; externalReference?: string; plan?: MarcenappPlan }) {
  return callAsaas<{ subscription: AsaasSubscription }>({ action: 'create_subscription', ...input });
}
