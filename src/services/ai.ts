...
export class AIAuthError extends Error { constructor(message = 'Faça login para usar os recursos de IA.') { super(message); this.name = 'AIAuthError'; } }
export class AIConfigError extends Error { constructor(message = 'A conexão com a IA não está configurada nesta publicação.') { super(message); this.name = 'AIConfigError'; } }
...
export const callAIFunction = async <T = unknown>(fn: string, body: unknown): Promise<T> => {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    try {
      const res = await fetch(`${SUPABASE_URL}/functions/v1/${fn}`, { method: 'POST', headers: { ...headers, 'x-retry-count': String(attempt) }, body: JSON.stringify(body) });
      ...
      if (transient && attempt < 2) { const retryAfter = Number(res.headers.get('Retry-After') ?? 2); await sleep(Math.min(Math.max(Number.isFinite(retryAfter) ? retryAfter : 2, 1), 10) * 1000); lastError = new Error(msg); continue; }
      throw new Error(msg);
    } catch (error) {
      ...
      if (attempt < 2 && /Failed to fetch|NetworkError|network/i.test(lastError.message)) { await sleep(500 * 2 ** attempt); continue; }
      throw lastError;
    }
  }
...
const imageParts = (images?: { mimeType: string; data: string }[]) => (images ?? []).filter((image) => image?.data).slice(0, 4).map((image) => ({ mimeType: image.mimeType || 'image/jpeg', data: image.data }));
...
export const callAIImage = async (prompt: string, images?: { mimeType: string; data: string }[]) => {
  ...
  const data = await callAIFunction<{ imageBase64?: string; imageUrl?: string | null }>('ai-image', { prompt, images: payloadImages });
...
export const callAIText = async (prompt: string, images?: { mimeType: string; data: string }[], jsonMode = false) => {
  ...
  const data = await callAIFunction<{ text: string }>('ai-text', { prompt, jsonMode, provider, images: imageParts(images) });
  return data.text;
};
