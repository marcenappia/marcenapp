...
import { buildTechnicalRenderPackage, isTechnicalRenderPackage, type TechnicalRenderPackage } from '@/lib/agents/renderContract';
...
export interface ExecutionContext { userId: string; projectId?: string; decorStyle?: string; lastImageBase?: string; lastImageMask?: string; technicalPackage?: TechnicalRenderPackage; }
...
type GerarRenderArgs = { prompt: string; estilo?: string; technicalPackage?: TechnicalRenderPackage };
...
const createProjeto: ToolDefinition<CreateProjetoArgs, ProjetoData> = { name: 'createProjeto', description: 'Cria projeto somente após confirmação explícita das três dimensões', version: '1.1.0', inputSchema: z.object({ nome: z.string().min(1), clienteNome: z.string().optional(), width: z.number().positive(), height: z.number().positive(), depth: z.number().positive(), tipo: z.string().optional(), confirmado: z.literal(true) }), async execute(args, ctx) { let clienteId: string | null = null; if (args.clienteNome) { const { data: cli } = await db.from('clientes').select('id').eq('user_id', ctx.userId).ilike('nome', args.clienteNome).maybeSingle(); clienteId = (cli as { id: string } | null)?.id ?? null; } const { data, error } = await db.from('projects').insert({ user_id: ctx.userId, nome: args.nome, cliente_id: clienteId, width: args.width, height: args.height, depth: args.depth }).select('id, nome, width, height, depth').single(); if (error) return { ok: false, error: error.message }; return { ok: true, data: data as ProjetoData }; } };

const gerarRender: ToolDefinition<GerarRenderArgs, RenderData> = {
  name: 'gerarRender', description: 'Enfileira render no Estúdio via Command Bus', version: '2.0.0',
  inputSchema: z.object({ prompt: z.string().min(1), estilo: z.string().optional(), technicalPackage: z.unknown().optional() }),
  async execute(args, ctx) {
    const estilo = args.estilo || ctx.decorStyle || 'Limpo';
    const technicalPackage = isTechnicalRenderPackage(args.technicalPackage)
      ? args.technicalPackage
      : ctx.technicalPackage;
    const referenceImages = technicalPackage?.referenceImages?.filter(image => Boolean(image.data)) ?? [];
    const legacyImages = ctx.lastImageBase && ctx.lastImageMask
      ? [{ mimeType: 'image/jpeg', data: ctx.lastImageBase }, { mimeType: 'image/png', data: ctx.lastImageMask }]
      : [];
    const images = referenceImages.length ? referenceImages : legacyImages;
    if (!technicalPackage && images.length === 0) return { ok: false, error: 'Pacote técnico/2D ausente e nenhuma imagem de referência disponível para o render.' };
    const id = useStudioStore.getState().enqueueCommand({
      prompt: `MARCENAPP IARA OS: móvel estilo ${estilo}. ${args.prompt}`,
      images,
      decor: estilo,
      metadata: { origin: 'iara', originalPrompt: args.prompt, targetModule: 'studio', technicalPackage: technicalPackage ?? undefined },
    });
    useMarcenappOS.getState().dispatchCommand({ source: 'iara', target: 'studio', action: 'GENERATE_VISUAL', payload: { prompt: args.prompt, estilo, studioCommandId: id, technicalPackage: technicalPackage ?? undefined } });
