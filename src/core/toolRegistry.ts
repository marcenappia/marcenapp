...
export interface ToolDefinition<TArgs, TResult = unknown> { name: string; description: string; version: string; inputSchema: z.ZodType<TArgs>; execute: (args: TArgs, ctx: ExecutionContext) => Promise<ToolResult<TResult>>; }
...
const calcularOrcamento: ToolDefinition<BudgetArgs> = {
  name: 'calcularOrcamento', description: 'Calcula orçamento do projeto atual usando o mesmo motor do módulo financeiro', version: '2.0.0',
...
const gerarContrato: ToolDefinition<ContractArgs> = {
  name: 'gerarContrato', description: 'Gera documentação contratual assistida por IA; não constitui aconselhamento jurídico', version: '1.1.0',
  ...
  async execute(args, ctx) {
    ...
    for (const desc of args.clausulasExtras ?? []) {
      try {
        ...
        if (text) { clausulas.push(text); const { error } = await supabase.from('custom_clauses').insert({ user_id: ctx.userId, clause_text: text, prompt: desc }); if (error) console.warn('Falha ao persistir cláusula:', error.message); }
...
const TOOLS: Record<string, ToolDefinition<never>> = { createCliente, createProjeto, gerarRender, calcularOrcamento, gerarContrato };
export function getTool(name: string) { return TOOLS[name]; }
export function listTools() { return Object.values(TOOLS); }
