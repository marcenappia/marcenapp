...
const createCliente: ToolDefinition = {
  ...
  async execute(args, ctx) {
    const { data, error } = await supabase.from('clientes').insert({ user_id: ctx.userId, nome: args.nome, email: args.email || null, telefone: args.telefone || null }).select('id, nome').single();
...
const calcularOrcamento: ToolDefinition = {
  ...
  async execute(_args, ctx) {
    ...
    const intMat = prices[proj.internal_material ?? 'mdf15_white'] || prices.mdf15_white; const extMat = prices[proj.external_material ?? 'mdf18_white'] || prices.mdf18_white;
    const frontalArea = Number(proj.height) * Number(proj.width); const sheetsInt = Math.ceil((frontalArea * 2.5 * 1.15) / intMat.area * 10) / 10; const sheetsExt = Math.ceil((frontalArea * 1.2 * 1.15) / extMat.area * 10) / 10; const costMat = sheetsInt * intMat.price + sheetsExt * extMat.price;
    const handleCost = (proj.handle_type ?? 'external') === 'external' ? ((proj.drawers ?? 4) + (proj.doors ?? 6)) * 15 : 0; const costHard = (proj.drawers ?? 4) * 28 + (proj.doors ?? 6) * 2 * 7.5 + handleCost + 200; const totalMat = costMat + costHard; const labor = totalMat * (Number(proj.labor_rate ?? 100) / 100); const subtotal = totalMat + labor + totalMat * 0.10; const total = subtotal * (1 + Number(proj.profit_margin ?? 35) / 100);
    return { ok: true, data: { projetoId: proj.id, nome: proj.nome, total: Number(total.toFixed(2)), materiais: Number(totalMat.toFixed(2)), maoDeObra: Number(labor.toFixed(2)), isEstimate: true } };
  },
};
