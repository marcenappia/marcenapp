...
const calcularOrcamento: ToolDefinition = {
  ...
  async execute(_args, ctx) {
    ...
    const result = calculatePricing({
      width: Number(proj.width), height: Number(proj.height), depth: Number(proj.depth), modules: proj.modules,
      drawers: proj.drawers, doors: proj.doors, internalMaterial: proj.internal_material, externalMaterial: proj.external_material,
      backMaterial: proj.back_material, handleType: proj.handle_type, profitMargin: proj.profit_margin, laborRate: proj.labor_rate,
    });
  ...
  async execute(args, ctx) {
    ...
    for (const desc of args.clausulasExtras ?? []) {
      try {
        ...
        if (text) {
          clausulas.push(text);
          const { error } = await supabase.from('custom_clauses').insert({ user_id: ctx.userId, clause_text: text, prompt: desc });
          if (error) console.warn('Falha ao persistir cláusula:', error.message);
        }
