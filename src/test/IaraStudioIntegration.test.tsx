...
vi.mock('@/integrations/supabase/client', () => ({
  supabase: { from: vi.fn(() => ({ select: vi.fn().mockReturnThis(), eq: vi.fn().mockReturnThis(), order: vi.fn().mockReturnThis(), limit: vi.fn().mockReturnThis(), maybeSingle: vi.fn(() => Promise.resolve({ data: { project_id: null, environment_id: null, version_id: null, last_correlation_id: null }, error: null })), insert: vi.fn(() => Promise.resolve({ error: null })) })) }
}));
...
describe('IARA-Studio Architecture', () => {
  ...
  const dispatchRender = (images?: { mimeType: string; data: string }[], scope = { projectId: 'A', environmentId: 'E1', versionId: 'V1' }) => {
    ...
    const osId = useMarcenappOS.getState().dispatchCommand({ source: 'iara', target: 'studio', action: 'GENERATE_VISUAL', payload: { prompt: 'Test', studioCommandId: studioId, userId: 'u1', ...scope, correlationId: 'corr-test', generation: 1 } });
  ...
  it('StudioWorker executes commands from the queue and syncs both stores', async () => {
    ...
    from.mockImplementation(() => ({ select: vi.fn().mockReturnThis(), eq: vi.fn().mockReturnThis(), order: vi.fn().mockReturnThis(), limit: vi.fn().mockReturnThis(), maybeSingle: vi.fn(() => Promise.resolve({ data: { project_id: 'A', environment_id: 'E1', version_id: 'V1', last_correlation_id: 'corr-test' }, error: null })), insert: vi.fn(() => Promise.resolve({ error: null })) }) as never);
  ...
  it('allows valid text-only IARA rendering without persisted project context', async () => {
    vi.mocked(studioService.generateVisual).mockResolvedValue('url-no-project');
    let ids: { studioId: string; osId: string } = { studioId: '', osId: '' };
    renderAct(() => { ids = dispatchRender(undefined, { projectId: null, environmentId: null, versionId: null }); });
    render(<StudioWorker />);
    await renderAct(async () => { await new Promise(r => setTimeout(r, 50)); });
    const osCmd = useMarcenappOS.getState().commandHistory.find(c => c.id === ids.osId);
    expect(osCmd?.status).toBe('completed');
    expect(osCmd?.result?.resultUrl).toBe('url-no-project');
    expect(studioService.generateVisual).toHaveBeenCalledTimes(1);
    expect(vi.mocked(studioService.generateVisual).mock.calls[0]?.[1]).toBeUndefined();
  });
  ...
  it('rejects an IARA render whose generation is missing', async () => {
    const from = vi.mocked((await import('@/integrations/supabase/client')).supabase.from);
    from.mockImplementation(() => ({ select: vi.fn().mockReturnThis(), eq: vi.fn().mockReturnThis(), order: vi.fn().mockReturnThis(), limit: vi.fn().mockReturnThis(), maybeSingle: vi.fn(() => Promise.resolve({ data: { project_id: 'A', environment_id: 'E1', version_id: 'V1', last_correlation_id: 'corr-missing' }, error: null })), insert: vi.fn(() => Promise.resolve({ error: null })) }) as never);
    ...
    expect(studioCmd?.status).toBe('cancelled');
  ...
  it('rejects an IARA render whose persisted correlation is stale after returning to the same project', async () => {
    ...
    const from = vi.mocked((await import('@/integrations/supabase/client')).supabase.from);
    from.mockImplementation(() => ({ select: vi.fn().mockReturnThis(), eq: vi.fn().mockReturnThis(), order: vi.fn().mockReturnThis(), limit: vi.fn().mockReturnThis(), maybeSingle: vi.fn(() => Promise.resolve({ data: { project_id: 'A', environment_id: 'E1', version_id: 'V1', last_correlation_id: 'corr-new-a' }, error: null })), insert: vi.fn(() => Promise.resolve({ error: null })) }) as never);
    render(<StudioWorker />);
    await renderAct(async () => { await new Promise(r => setTimeout(r, 50)); });
    const osCmd = useMarcenappOS.getState().commandHistory.find(c => c.id === osId);
    const studioCmd = useStudioStore.getState().commandQueue.find(c => c.id === studioId);
