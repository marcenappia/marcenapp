...
describe('Onboarding Component', () => {
  ...
  it('renders and persists progress across steps', async () => {
    renderOnboarding(); expect(screen.getByText(/Bem-vindo ao MarcenApp!/i)).toBeInTheDocument();
  ...
  it('restores progress from localStorage when not logged in', async () => {
    ...
    renderOnboarding('chat'); expect(screen.getByText(/Studio 3D/i)).toBeInTheDocument();
  });
  it('handles spotlight rendering for modules', async () => {
    const div = document.createElement('div'); div.id = 'nav-studio'; document.body.appendChild(div);
    localStorage.setItem('marcenapp_onboarding_step', '2'); renderOnboarding('chat');
