...
export async function guardRequest(
  req: Request,
  cors: Record<string, string>,
  opts: { fn: string; limit: number; windowSeconds: number },
) {
  const authHeader = req.headers.get("Authorization") ?? "";
  const token = authHeader.startsWith("Bearer ") ? authHeader.slice(7).trim() : "";
  if (!token) return { ok: false, response: jsonResponse(cors, { error: "Autenticação necessária." }, 401) };
  ...
  if (rlError) {
    return {
      ...
      response: jsonResponse(
        cors,
        { error: "Não foi possível validar o limite de uso da IA.", code: "rate_limit_unavailable" },
        503,
        { "Retry-After": "10" },
      ),
    };
  ...
  if (!row || row.allowed === false) {
    ...
    return {
      ...
      response: jsonResponse(
        cors,
        { error: row ? `Limite de uso atingido. Tente novamente em ${retry}s.` : "Não foi possível validar o limite de uso da IA.", retryAfterSeconds: retry },
        429,
...
export async function readJsonBody(req: Request, maxBytes: number) {
  ...
  if (declared > maxBytes) return { ok: false, reason: "too_large" as const };
  const text = await req.text();
  if (text.length > maxBytes) return { ok: false, reason: "too_large" as const };
  try {
    return { ok: true, body: JSON.parse(text) } as const;
  } catch {
