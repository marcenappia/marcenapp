...
const TOOL_DECLARATIONS = [
  ...
  { name: "iaraSmartAction", description: "Executa uma ação contextual da IARA usando dados reais do projeto. Use para Materiais, Ferragens, Plano de corte, Estoque, Produção, Orçamento, Documentos, Pedido, Montagem, Instalação, Checklist, Entrega, Revisar projeto ou Conferir medidas. Nunca invente dados; se não houver projeto, a execução será bloqueada.", parameters: { type: "object", properties: { action: { type: "string", enum: ["materials", "hardware", "inventory", "production", "cut", "budget", "documents", "order", "assembly", "installation", "checklist", "delivery", "review_project", "check_measurements"], description: "Ação contextual solicitada" }, projectId: { type: "string", description: "ID do projeto atual, quando conhecido" } }, required: ["action"] } },
];
...
- Para ações da IARA como materiais, ferragens, corte, estoque, produção, orçamento, documentos, pedido, montagem, instalação, checklist, entrega, revisão e conferência de medidas, use iaraSmartAction quando a intenção estiver clara. Passe o projectId do contexto quando existir.
