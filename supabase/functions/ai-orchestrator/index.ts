...
const TOOL_DECLARATIONS = [
  ...
  { name: "createProjeto", description: "Cria um projeto de marcenaria diretamente a partir de texto, sem exigir imagem. Use somente quando nome, largura, altura e profundidade estiverem explícitos. Não invente dimensões; se faltar alguma, peça somente a informação que falta.", parameters: { type: "object", properties: { nome: { type: "string" }, clienteNome: { type: "string" }, width: { type: "number" }, height: { type: "number" }, depth: { type: "number" }, tipo: { type: "string" }, confirmado: { type: "boolean" } }, required: ["nome", "width", "height", "depth", "confirmado"] } },
