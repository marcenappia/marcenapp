...
const TOOL_DECLARATIONS = [
  ...
  { name: "createProjeto", description: "Cria um projeto de marcenaria diretamente a partir de texto, sem exigir imagem. Pode ser chamado quando o usuário, na própria mensagem ou no contexto imediato da conversa, fornecer nome e as três dimensões (largura, altura e profundidade) de forma explícita e pedir para criar o projeto. Não invente nenhuma dimensão. Se faltar qualquer dimensão, peça somente a informação que falta.", parameters: { type: "object", properties: { nome: { type: "string", description: "Nome do projeto" }, clienteNome: { type: "string", description: "Cliente vinculado, se informado" }, width: { type: "number", description: "Largura em metros, conforme informado pelo usuário" }, height: { type: "number", description: "Altura em metros, conforme informado pelo usuário" }, depth: { type: "number", description: "Profundidade em metros, conforme informado pelo usuário" }, tipo: { type: "string", description: "Tipo do móvel" }, confirmado: { type: "boolean", description: "Use true quando o usuário tiver fornecido explicitamente as três dimensões e solicitado a criação do projeto; nunca use true com dimensão inventada ou ausente" } }, required: ["nome", "width", "height", "depth", "confirmado"] } },
  { name: "gerarRender", description: "Solicita materialização visual. Um render baseado no ambiente exige imagem base e máscara; a criação do projeto em si não exige imagem e deve ser feita por texto quando houver dados suficientes.", parameters: { type: "object", properties: { prompt: { type: "string", description: "Descrição do que renderizar" }, estilo: { type: "string", description: "Estilo visual" } }, required: ["prompt"] } },
...
- Um render baseado em uma fotografia do ambiente pode exigir imagem; isso é diferente de criar o projeto por texto.
