...
const cors = (req: Request) => {
  ...
  return { ...corsHeaders, "Access-Control-Allow-Origin": allowed, "Access-Control-Allow-Methods": "GET, POST, OPTIONS", Vary: "Origin" };
};
...
async function getActiveProduct(productKey: string) {
  ...
  if (!data || Number(data.price_cents) <= 0 || Number(data.credits) <= 0) throw Object.assign(new Error("product_not_available"), { status: 409 });
  return data as { code: string; name: string; description: string | null; operation_type: string; credit_type: string; credits: number; price_cents: number; status: string };
}
...
async function ensurePurchaseRecord(userId: string, product: { code: string; name: string; credit_type: string; credits: number; price_cents: number }, customerId: string, externalReference: string) {
  const a = admin();
  const { data: existing, error: lookupError } = await a.from("billing_purchases").select("id,asaas_payment_id,status,credits_granted_at,product_key,product_name,credit_type,credits,amount,asaas_customer_id,external_reference").eq("user_id", userId).eq("external_reference", externalReference).maybeSingle();
  ...
  const { data, error } = await a.from("billing_purchases").insert(row).select("id,asaas_payment_id,status,credits_granted_at,product_key,product_name,credit_type,credits,amount,asaas_customer_id,external_reference").single();
  if (!error) return data;
  if (String(error.code ?? "") === "23505") {
    const { data: raced } = await a.from("billing_purchases").select("id,asaas_payment_id,status,credits_granted_at,product_key,product_name,credit_type,credits,amount,asaas_customer_id,external_reference").eq("user_id", userId).eq("external_reference", externalReference).maybeSingle();
...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "webhook") {
      ...
      return await processWebhook(h, body as Record<string, unknown>, req.headers.get("asaas-access-token")?.trim());
    }
    ...
    if (action === "create_product_payment") {
      ...
      return json(h, { payment, product, checkoutUrl, reused: false });
    }
    ...
    if (action === "create_subscription") {
      ...
      const idempotencyKey = String(body.idempotencyKey ?? "").trim();
      if (!customer || !validPlans.includes(plan as never) || !idempotencyKey || idempotencyKey.length > 128) return json(h, { error: "Cliente, plano e idempotencyKey válidos são obrigatórios." }, 400);
      ...
      return json(h, { subscription, plan: planRow, checkoutUrl, reused: false });
    }
