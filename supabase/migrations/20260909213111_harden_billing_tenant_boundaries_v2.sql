...
begin
  foreach v_table in array array[
    'public.agent_skills_registry',
    'public.profiles',
    'public.projects',
    'public.gallery_images',
    'public.custom_clauses',
    'public.clientes',
    'public.user_roles',
    'public.chat_messages',
    'public.ai_provider_settings',
    'public.orchestrator_runs',
    'public.account_trials',
    'public.billing_customers',
    'public.diario_entradas'
  ] loop
    if to_regclass(v_table) is not null then
      execute 'revoke all on table ' || v_table || ' from anon';
    end if;
  end loop;
end;
$$;
...
do $$
begin
  if to_regclass('public.diario_entradas') is not null then
