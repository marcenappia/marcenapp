create table if not exists public.project_cost_snapshots (
  ...
  constraint project_cost_nonnegative check ((sale_price is null or sale_price >= 0) and (material_cost is null or material_cost >= 0) and (hardware_cost is null or hardware_cost >= 0) and (labor_cost is null or labor_cost >= 0) and (other_cost is null or other_cost >= 0)),
